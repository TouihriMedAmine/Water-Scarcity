...
DATA_DIR = os.path.join(os.path.dirname(__file__), "../visualization_outputUp_2019_2024/AvgSurfT")
MODEL_PATH = os.path.join(os.path.dirname(__file__), "modele_unet_AvgSurfT.h5")
...
def extract_date_from_filename(filename):
    """Extrait la date du nom de fichier (ex: AvgSurfT_20190101.png)"""
...
def find_jmoins1_image_path(date_cible):
    ...
    filename = f"AvgSurfT_{date_jmoins1.strftime('%Y%m%d')}.png"
...
def predict():
    ...
    physical_prediction = MAX_PHYSICAL_VALUE+23.3 - pred_norm * (MAX_PHYSICAL_VALUE-MIN_PHYSICAL_VALUE)
    return jsonify({"prediction": physical_prediction})
